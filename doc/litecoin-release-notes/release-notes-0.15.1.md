nickoin Core version *0.15.1* is now available from:

  <https://download.nickoin.org/nickoin-0.15.1/>
...
A number of changes to the way nickoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 nicks per kB (0.00001 NCK). This is to prepare for dropping the minimum transaction fee to 0.1 nicks per kB (0.0001 NCK) in the next release.
