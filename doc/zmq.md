...
The nickoin Core daemon can be configured to act as a trusted "border
router", implementing the nickoin wire protocol and relay, making
...
The ZeroMQ feature in nickoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nickoind:
...
For instance:
    ...
    $ nickoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/nickoind.tx.raw \
...
These options can also be provided in nickoin.conf.
...
From the perspective of nickoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nickoind directly. Furthermore, no information is
...
using. nickoind appends an up-counting sequence number to each
